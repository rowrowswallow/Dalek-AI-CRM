...
这个账号带 24 家客户的完整数据，方便你先把功能点一遍。

### 让别人用（邀请码注册）

要在「设置 → 账号与邀请」里生成邀请码，然后把链接发给对方：

```
http://你的地址/?invite=XXXXXXXX
```

对方打开链接，填账号密码就能自己注册。注册后系统会给他一份**独立的数据**（8 家客户、几个任务、一整套知识库），和你的完全隔开——他改坏了不影响你，你也看不到他的。

邀请码可以设置可用次数和有效期，随时能停用或删除。

在同一个页面还能看到已注册的用户，删掉某人会连同他的数据一起清掉。

### 安全说明

- 密码用 scrypt 加盐哈希存储，数据库里没有明文
- 登录令牌存在数据库里，带 30 天有效期，重启服务不会掉登录
- 所有数据接口都要求登录，未登录一律返回 401
- 所有业务数据按工作区隔离，跨区读写会被拒绝
