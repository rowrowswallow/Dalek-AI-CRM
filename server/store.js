...
const api = {
  customers(filter) {
    let rows = db.prepare('SELECT * FROM customers ORDER BY created_at DESC').all().map(rowCustomer);
  ...
  followups(filter) {
    const rows = (filter && filter.customer_id)
      ? db.prepare('SELECT * FROM followups WHERE customer_id = ? ORDER BY created_at DESC LIMIT 50').all(filter.customer_id)
      : db.prepare('SELECT * FROM followups ORDER BY created_at DESC LIMIT 200').all();
  ...
  addFollowup(d) {
    ...
    db.prepare(`INSERT INTO followups
      ...
      d.source || 'manual', d.created_by || '我', new Date().toISOString());
    if (c) {
  ...
  tasks(filter) {
    let rows = db.prepare('SELECT * FROM tasks ORDER BY created_at DESC').all().map(rowTask);
    if (filter && filter.customer_id) rows = rows.filter((t) => t.customer_id === filter.customer_id);
    return rows;
  },

  taskCounts() {
    const r = db.prepare(`SELECT COUNT(*) AS a,
      SUM(CASE WHEN status='todo' THEN 1 ELSE 0 END) AS t,
      SUM(CASE WHEN status='doing' THEN 1 ELSE 0 END) AS d,
      SUM(CASE WHEN status='done' THEN 1 ELSE 0 END) AS dn FROM tasks`).get();
  ...
  topPriority(n = 5) { return api.customers().slice(0, n); },

  stats() {
    const all = api.customers();
    const tc = api.taskCounts();
    const weekAgo = new Date(nowMs - 7 * 864e5).toISOString();
    const followWeek = db.prepare('SELECT COUNT(*) AS c FROM followups WHERE created_at >= ?').get(weekAgo).c;
    const followTotal = db.prepare('SELECT COUNT(*) AS c FROM followups').get().c;
    ...
    db.prepare('SELECT created_by, COUNT(*) AS c FROM followups WHERE created_at >= ? GROUP BY created_by')
      .all(weekAgo).forEach((r) => { if (byOwner[r.created_by]) byOwner[r.created_by].follow = r.c; });
